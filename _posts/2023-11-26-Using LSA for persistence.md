...
What may surprise you is that LSA can also be used for persistence, where our code will be executed inside the lsass's process memory. Consequently, our code will run as the SYSTEM user and we will be able to extract credentials from the same address space. All of these techniques involve creating DLLs to be loaded by LSA into the lsass process. 
